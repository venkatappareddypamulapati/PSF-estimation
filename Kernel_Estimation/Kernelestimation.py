...
def kernel_estimation(I, B, lens=5, lam=0.1, method = 'l1ls', verbose = True):
#    
#    [h,w] = I.shape
#    if h<100 and w <100:
#        # row = range(0,h)
...
#        A_sparse = csr_matrix(A)
        ...
        # K = K*M_high
        #K=np.random.rand([lens**2,1])
        for i in range(1,niter+1):
            # K = K*M_low
            K = K + beta * (Atb - (AtA + lambd2*E).dot(K))
            #K=K-beta*(A*np.tanh(1*(A*K-b))+(lambd2*K))
            #K=K-beta*(A*np.tanh(1*(At*K-b))+lambd2*K)-(0.5*np.sign(K))/(1+0.1*np.abs(K))
            K[K<0] = 0
            K = K/K.sum()
