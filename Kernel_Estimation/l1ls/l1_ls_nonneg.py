...
def l1ls_nonneg(A, y, lmbda=0.1, x0=None, At=None, m=None, n=None, tar_gap=1e-3,
    ...
    * S.-J. Kim, K. Koh, M. Lustig, S. Boyd, and D. Gorinevsky. An
    ...
    BETA = 0.1         # stepsize decrease factor
    ...
    for ntiter in range(0, MAX_NT_ITER):
        ...
        #pobj = norm(z) + lmbda*np.sum(x)
        
        pobj=z.dot(z)+lmbda*np.sum(x)
        
        #pobj = np.log(np.cosh(1*z)).sum() + lmbda*np.sum(x)
        ...
        #phi = np.log(np.cosh(z)).sum() + lmbda*np.sum(x) - np.sum(np.log(-f)) / t
        s = 1.0
        gdx = gradphi.dot(dx)
